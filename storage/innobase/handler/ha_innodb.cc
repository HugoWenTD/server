	...
	int kill_signal)
{
	...
	if (victim_trx) {
		...
		/* Transaction must be active state when killing it,
		it may not be committed in memory or prepared */
		if (victim_trx->state == TRX_STATE_ACTIVE) {
			...
			WSREP_DEBUG("Victim trx id: "  TRX_ID_FMT
				    " in trx_state: %d",
				    victim_trx->id,
				    victim_trx->state);
		}
