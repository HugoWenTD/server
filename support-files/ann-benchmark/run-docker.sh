...
#
# Usage:
#   [MARIADB_SOURCE_DIR=.] [ANN_WORKSPACE=./ann-workspace] [DOCKER_REBUILD=yes] [MARIADB_REBUILD=yes] ./support-files/ann-benchmark/run-docker.sh
#
...
docker run -it --rm --user $(id -u):$(id -g) -v $MARIADB_SOURCE_DIR:/build/server -v $ANN_WORKSPACE:/build/ann-workspace $IMAGE_NAME /bin/bash -c "$cmd"
